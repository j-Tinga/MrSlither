...
public class PlayGameState extends BasicGameState{
    ...
    public void render(GameContainer gc, StateBasedGame sbg, Graphics g) throws SlickException {
        ...
        //
        diet.getObjectImage().draw((float) diet.getObjectPosition().getX(),(float) diet.getObjectPosition().getY());//render image of diet
    ...
    public void update(GameContainer gc, StateBasedGame sbg, int i) throws SlickException {
       ...
       //COLLISIONS 
        ...
        if(buffer == 4 && died != true){    //change Snake Speed through buffer ( Speed = fps/buffer, current snake moves at 7.5 tiles per second)
            ...
            }if(snake.getHeadHitbox().intersects(diet.getObjectHitbox())){    //Check if Snake Collides with diet
               
                freePos = findFreeSpace();  //Finds a free space in the board
                diet.moveObject(freePos);  //Moves the diet to new free space
                snake.goDiet(diet);    //Snake eats diet and shortens
                diet.decreaseMoney(money);// decreases the current money
            }
                  buffer=0;      
